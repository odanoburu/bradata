...
def get_cgu_data(date, cadastro, freq, consulta=None):
    ...
    :param consulta: usually the same as in cadastro, but sometimes the internal API calls it something else, as in the case of CEAF.
    ...
    time_dict = {'a': ['a', 'consulta'], 'm': ['a', 'm', 'consulta'], 'd': ['d', 'm', 'a', 'consulta'], 'y': ['a', 'consulta']}
    ...
    r = requests.get('http://arquivos.portaldatransparencia.gov.br/downloads.asp', stream=True, params=filtered_params, timeout=1)
...
def get_diarias(date=None):
    """
    gets pagamentos de diárias pagas aos servidores e colaboradores eventuais (http://www.portaltransparencia.gov.br/despesasdiarias/) data. it
    converts the csv encoding to utf8.
    :param date: a string in YYYY-mm format or a datetime object with year and month attributes. if not provided, will get current day (be
    careful if on other timezone than Brasília). input can be constructed by 
    importing datetime module and typing `datetime.date(1994, 07, 18)`. 
    :return: downloads csv to directory bradata.__download_dir__
    """
